...
bin50=np.array(range(-1000,2600,250)) #-1ps to 2ps, 739109-123,127-135,175-180,254,255
...
for i, scan in enumerate(items):
	...
	with h5py.File('C:/data/sacla/VtC_'+str(scan)+'.h5','r') as f: #which emission line?
		...
		#shot-to-shot counts on VHS detector at pixel number 90 to 170
		sts=np.average(np.array(tempset).transpose()[90:170], axis=0)
		...
		for k, pulse in enumerate(tempset): #outliers rejection, sts 3000 to 25000 for Kb, 60 to 1500 for VtC, 5000 to 10000 for Ka
			#bkgset.append(pulse)
			if I1[k]<0.01 or math.isnan(tma[k]) or tma[k]>1300 or tma[k]<600 or I1[k]/I0[k]<avrI10-0.01 or I1[k]/I0[k]>avrI10+0.01 or sts[k]<60 or sts[k]>1500:
				filtered+=1
			elif onoff[k]==1:
				if pump[k]!=0:
		...
		for e, zeit in enumerate(on[0]): #re-binning
			if -1500<=zeit<=19500: #our hard-limit of re-binning for -1ps to 19ps
				if -1125<=zeit<=2625: #our hard-limit of re-binning for -1ps to 2ps
					bin50set[[round(l) for l in (bin50-zeit)/250].index(0)].append(on[1][e])
					pump50[[round(l) for l in (bin50-zeit)/250].index(0)].append(on[2][e])
...
#np.savetxt('C:/data/sacla/vtc_bkg.txt',(np.average(bkgset,axis=0)))
...
#avr19error=np.std(delay19,axis=0)/np.sqrt(len(delay19))
#avr19p=[np.average(pump19, axis=0),np.std(pump19,axis=0)/np.sqrt(len(pump19))]
#print(avr19p)
#plt.errorbar(list(range(len(offavr))),avr19-offavr,np.sqrt(avr19error**2+offerror**2))
#plt.show()
#np.savetxt('C:/data/sacla/absolute_19psVtC_739134-159,181-255.txt', (offavr, avr19))
...
for diff in diff50:
    ...
    plt.show()
'''
#np.savetxt('C:/data/sacla/-1ps_to_2ps_errorbar_250fsbin_areanorm_739109-123,127-135,175-180,254,255_offerror.txt', (offerror))
#np.savetxt('C:/data/sacla/-1ps_to_2ps_errorbar_250fsbin_areanorm_739109-123,127-135,175-180,254,255_onerror.txt', (avr50error))
#np.savetxt('C:/data/sacla/-1ps_to_2ps_errorbar_250fsbin_areanorm_739109-123,127-135,175-180,254,255_differror.txt', (diff50error))
#np.savetxt('C:/data/sacla/-1ps_to_2ps_errorbar_250fsbin_areanorm_739109-123,127-135,175-180,254,255_traceerror.txt', ([np.sqrt(np.sum(u[90:115]**2)) for u in diff50error]))

#np.savetxt('C:/data/sacla/-1ps_to_2ps_Cudmp_VtC500fs_absolute.txt', ([bin50, count50, avr50p]))
#np.savetxt('C:/data/sacla/-1ps_to_2ps_Cudmp_VtC500fs_absolute_on.txt', (avr50))
#np.savetxt('C:/data/sacla/-1ps_to_2ps_Cudmp_VtC500fs_absolute_off.txt', (offavr))
#np.savetxt('C:/data/sacla/-1ps_to_2ps_Cudmp_VtC500fs_absolute_tr.txt', ([bin50, [np.trapz(af[80:115]) for af in diff50]]))

#plt.plot(bin500, [np.trapz(t[80:115]) for t in diff500], 'bo')
#plt.show()
plt.errorbar(bin50, [np.trapz(u[90:115]) for u in diff50], [np.sqrt(np.sum(u[90:115]**2)) for u in diff50error])
plt.show()
