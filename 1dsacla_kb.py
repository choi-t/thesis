...
bin500=np.array(range(-1000,19500,500)) #-1ps to 19ps, 739134,135,254,255
...
for i, scan in enumerate(items):
	...
	with h5py.File('C:/data/sacla/Kb_'+str(scan)+'.h5','r') as f: #which emission line?
		...
		if len(xfelonoff)!=len(onoff) or uxfel[0]==0:
			print("!!!Pulse selector of xfel and laser don't match, or includes xfel=0")
		...
		#shot-to-shot counts on VHS detector at pixel number 215 to 234
		sts=np.average(np.array(tempset).transpose()[215:234], axis=0)
		...
		for k, pulse in enumerate(tempset): #outliers rejection, sts 7500 to 17500 for Kb, 60 to 1500 for VtC, 5000 to 10000 for Ka
			if I1[k]<0.01 or math.isnan(tma[k]) or tma[k]>1300 or tma[k]<600 or I1[k]/I0[k]<avrI10-0.01 or I1[k]/I0[k]>avrI10+0.01 or sts[k]<7500 or sts[k]>17500:
				filtered+=1
			elif onoff[k]==1:
				if pump[k]!=0:
		...
		#offset1+=off
		#plt.plot(offset[i]/np.trapz(offset[i]))
		#plt.show()

		oncounts+=len(on[1])

		for e, zeit in enumerate(on[0]): #re-binning
			if -1250<=zeit<=19250: #our hard-limit of re-binning for -1ps to 19ps
				...
				bin500set[[round(m) for m in (bin500-zeit)/500].index(0)].append(on[1][e])
				pump500[[round(m) for m in (bin500-zeit)/500].index(0)].append(on[2][e])
			else:
				pass
...
offavr=np.average(offset, axis=0)/np.trapz(np.average(offset, axis=0))
#offerror=np.std(offset1,axis=0)/np.sqrt(len(offset1))/np.trapz(np.average(offset1, axis=0))

for offscan in offset:
	plt.plot(offscan/np.trapz(offscan)-offset[-1]/np.trapz(offset[-1]))
	plt.show()
...
#plt.show()
#np.savetxt('C:/data/sacla/absolute_19psKb_739134-159,181-255.txt', (offavr, avr19))
#print(avr19p)
...
for diff in diff500:
    ...
    plt.show()
'''
#np.savetxt('C:/data/sacla/-1ps_to_20ps_errorbar_kb500_areanorm_739134,135,254,255_offerror.txt', (offerror))
#np.savetxt('C:/data/sacla/-1ps_to_20ps_errorbar_kb500_areanorm_739134,135,254,255_onerror.txt', (avr500error))
#np.savetxt('C:/data/sacla/-1ps_to_20ps_errorbar_kb500_areanorm_739134,135,254,255_differror.txt', (diff500error))
#np.savetxt('C:/data/sacla/-1ps_to_20ps_errorbar_kb500_areanorm_739134,135,254,255_traceerror.txt', ([np.sqrt(np.sum(u[180:218]**2)+np.sum(u[242:280]**2)) for u in diff500error]))

#np.savetxt('C:/data/sacla/-1ps_to_20ps_Cudmp_Kb0.5ps_areanorm_739134,135,254,255_absolute.txt', ([bin500, count500, avr500p]))
#np.savetxt('C:/data/sacla/-1ps_to_20ps_Cudmp_Kb0.5ps_areanorm_739134,135,254,255_absolute_on.txt', (avr500))
#np.savetxt('C:/data/sacla/-1ps_to_20ps_Cudmp_Kb0.5ps_areanorm_739134,135,254,255_absolute_off.txt', (offavr))
#np.savetxt('C:/data/sacla/-1ps_to_20ps_Cudmp_Kb0.5ps_areanorm_739134,135,254,255_absolute_tr.txt', ([bin500, [np.trapz(af[180:218])+np.trapz(af[242:280]) for af in diff500]]))

plt.plot(bin500, [np.trapz(t[180:218])+np.trapz(t[242:280]) for t in diff500], 'bo')
#plt.show()
#plt.errorbar(bin500, [np.trapz(u[180:218])+np.trapz(u[242:280]) for u in diff500], [np.sqrt(np.sum(u[180:218]**2)+np.sum(u[242:280]**2)) for u in diff500error])
#plt.errorbar(bin50, [np.trapz(u[221:238]) for u in diff50], [np.sqrt(np.sum(u[221:238]**2)) for u in diff50error])
plt.show()
